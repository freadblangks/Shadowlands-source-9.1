...
struct npc_private_cole : public ScriptedAI
{
    ...
    void QuestAccept(Player* player, Quest const* quest) override
    {
        ...
        {
            if (Creature* npc = me->SummonCreature(me->GetEntry(), me->GetPosition(), TEMPSUMMON_MANUAL_DESPAWN, 0U, 0U))
            {
                npc->SetFaction(14);
        ...
        if (Player* player = attacker->ToPlayer())
            if (me->GetHealth() < me->CountPctFromMaxHealth(20)) // the intent is not to kill but to sparr
                if (player->HasQuest(QUEST_STAND_YOUR_GROUND))
                {
                    ...
                    me->SummonCreature(me->GetEntry(), Position(-52.6575f, 0.689299f, 43.4511f, 4.12704f), TEMPSUMMON_MANUAL_DESPAWN, 0U, 0U);
                }
    }
        ...
        if (Player* player = attacker->ToPlayer())
            if (me->GetHealth() < me->CountPctFromMaxHealth(50)) // the intent is not to kill but to sparr
                if (player->HasQuest(QUEST_ENHANCED_COMBAT_TACTICS))
                {
                    ...
                    me->SummonCreature(me->GetEntry(), me->GetPosition(), TEMPSUMMON_MANUAL_DESPAWN, 0U, 0U);
                    me->DespawnOrUnsummon(10ms);
                }
    }
...
struct npc_captain_garrick_156280 : public ScriptedAI
{
    ...
    {
        if (quest->GetQuestId() == QUEST_RIDE_OF_THE_SCIENTIFICALLY_ENHANCED_BOAR)
            player->SummonCreature(156595, player->GetPosition(), TEMPSUMMON_MANUAL_DESPAWN, 60000, 0U, player->GetGUID()); //10 minutes to despawn
    }
...
struct npc_alaria_175031 : public ScriptedAI
{
    ...
    void QuestAccept(Player* player, Quest const* quest) override
    {
        ...
        {
            if (Creature* npc = player->SummonCreature(156651, Position(-249.059f, -2492.52f, 18.0742f, 0.659374f), TEMPSUMMON_MANUAL_DESPAWN, 0U, 0U))
            {
...
struct npc_geolord_grekog_151091 : public ScriptedAI
{
    ...
    void JustDied(Unit* attacker) override
    {
        if (Player* player = attacker->ToPlayer())
        {
            ...
            {   // lindie springstock
                if (Creature* npc = player->SummonCreature(149899, Position(16.4271f, -2511.82f, 78.8215f, 5.66398f), TEMPSUMMON_MANUAL_DESPAWN, 0U, 0U))
                {
    ...
    void JustDied(Unit* attacker) override
    {
        if (Player* player = attacker->ToPlayer())
        {
            if (player->HasQuest(QUEST_WHO_LURKS_IN_THE_PIT))
            {
                ...
                // ralia mount
                if (Creature* npc2 = player->SummonCreature(156929, player->GetPosition(), TEMPSUMMON_MANUAL_DESPAWN, 60000, 0U)) //10 minutes to despawn
                {
